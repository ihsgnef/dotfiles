...
for s = 1, screen.count() do
	...
	tags[s] = awful.tag({ " web ", " term ", "android", " vim ", " pdf ", " misc ", " file ", " win " }, s, layouts[1])
end
-- }}}
			...
			function ()
				...
				awful.prompt.run({
					...
					function(word)
						...
						f:close()
						naughty.notify({ title = "<span color='red'>" .. word .. "</span>:", text = '<span font_desc="Sans 9">' .. fr ..'</span>', timeout = 15, width = 400, screen = mouse.screen })
					end)
				end)
				)
				...
				naughty.config.default_preset.font             = beautiful.font or "Sans 9"
